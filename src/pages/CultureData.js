const cultureData = [
    {
        id: "1",
        title: "Penang Temple",
        location: "Kek Lok Si Temple",
        description: "A beautiful Buddhist temple perched on the hills of Air Itam, Penang.",
        image: "/temple.jpg",
        additionalVideo: "/temple.mp4",
        siteLink: "https://www.kekloksitemple.com/",
        googleMapsLink:"https://www.google.com/maps/place/kek+lok+si+temple/data=!4m2!3m1!1s0x304ac2185dfc8665:0x2c9084ea7f433ec4?sa=X&ved=1t:155783&ictx=111",
        fullDescription: `
      <p>The Kek Lok Si Temple, located in Air Itam, Penang, is one of the largest and most important Buddhist temples in Southeast Asia. Established in 1890, it has become an iconic symbol of Penang's rich cultural heritage. The temple's sprawling complex consists of several halls, pavilions, and gardens, featuring intricate carvings, statues, and colorful decorations. The highlight of the temple is the towering statue of the Goddess of Mercy, Kuan Yin, which stands at 30 meters tall.</p>
      
      <p>The temple also houses a beautiful pagoda that combines Chinese, Thai, and Burmese architectural styles. As you walk around the temple grounds, you will encounter various statues of Buddha and other deities, as well as the iconic lanterns that are lit during Chinese New Year festivals. The temple attracts both locals and tourists alike, who come to pray, make offerings, or simply enjoy the peaceful atmosphere and stunning views of the surrounding area.</p>
      
      <ul>
        <li>Explore the temple's grand architecture and serene ambiance.</li>
        <li>Visit the statue of Kuan Yin and the famous pagoda.</li>
        <li>Participate in the annual Chinese New Year celebrations, where the temple is beautifully decorated with lanterns.</li>
      </ul>
    `,
    },
    {
        id: "2",
        title: "Penang Street Art",
        location: "Penang Street Art",
        description: "A vibrant collection of murals and interactive art in the streets of George Town.",
        image: "/streetart.jpg",
        additionalVideo: "/streetart.mp4",
        siteLink: "https://www.streetartpenang.com/",
        googleMapsLink:"https://www.google.com/maps/place/penang+street+art/data=!4m2!3m1!1s0x304ac3926b8cc0f9:0xe76a8f58f4205b17?sa=X&ved=1t:155783&ictx=111",
        fullDescription: `
      <p>Penang Street Art is one of the most famous and unique attractions in George Town, Penang. Since 2012, the streets of George Town have been transformed into an outdoor gallery, featuring a series of murals and art installations by renowned local and international artists. The street art is not only a visual delight but also tells the story of Penang's culture, history, and people.</p>

      <p>Among the most famous pieces is a series of murals by Lithuanian artist Ernest Zacharevic, which depicts children interacting with the surroundings in a playful and often humorous manner. In addition to the murals, the streets also feature interactive 3D art installations that allow visitors to take creative and fun photographs. Penang Street Art is a vibrant fusion of tradition and modernity, making it a must-visit destination for art lovers and tourists alike.</p>

      <ul>
        <li>Discover murals by Ernest Zacharevic and other renowned artists.</li>
        <li>Interact with 3D art installations for a fun and memorable experience.</li>
        <li>Learn about the stories and history behind the artwork through guided tours.</li>
      </ul>
    `,
    },
    {
        id: "3",
        title: "George Town",
        location: "George Town, Penang",
        description: "A UNESCO World Heritage Site filled with cultural history, colonial architecture, and street food.",
        image: "/gtw.jpg",
        additionalVideo: "/gtw.mp4",
        siteLink: "https://en.wikipedia.org/wiki/George_Town,_Penang",
        googleMapsLink:"https://www.google.com/maps/place/George+Town,+Penang/@5.405964,100.2681468,13z/data=!3m1!4b1!4m6!3m5!1s0x304ac397ad2b7bd5:0x239ae45978a9b934!8m2!3d5.4140975!4d100.3285271!16zL20vMDE1ODA2?entry=ttu&g_ep=EgoyMDI1MDEwMi4wIKXMDSoASAFQAw%3D%3D",
        fullDescription: `
      <p>George Town is one of Malaysia's most charming and historically significant cities. Established in 1786, it has been a melting pot of cultures, with influences from the Chinese, Malay, Indian, and European communities. The city’s colonial architecture, bustling markets, and narrow streets give it a unique and old-world charm that transports visitors back in time.</p>

      <p>George Town’s UNESCO World Heritage status ensures that its historic buildings and streetscapes are well-preserved. The city is home to beautiful old shophouses, mansions, and temples, which are still in use today. Some of the must-visit landmarks include Fort Cornwallis, the Penang Peranakan Mansion, and the Khoo Kongsi clan house. Additionally, George Town is famous for its food, with a mix of traditional hawker fare like Char Koay Teow, Penang Laksa, and Nasi Kandar, which reflects the city’s multicultural history.</p>

      <ul>
        <li>Wander through George Town's historic streets, filled with colonial-era buildings and Chinese shophouses.</li>
        <li>Visit iconic landmarks such as Fort Cornwallis, Penang Peranakan Mansion, and Khoo Kongsi clan house.</li>
        <li>Indulge in Penang's famous street food, including Char Koay Teow, Penang Laksa, and Nasi Kandar.</li>
      </ul>
    `,
    },
];

export default cultureData;
